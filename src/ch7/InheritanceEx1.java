...
public class InheritanceEx1 {
	...
	/*
	 *  상속(Inheritance)
	 *   - OOP(객체지향프로그래밍)에서 상속은 기존의 클래스를 확장하여 새로운 클래스를 이끌어 내는 것을 의미함.
	 *   - 기존 코드를 재사용함으로써 불필요한 코드를 재작성하지 않아도 됨.
	 *   - 클래스의 상속은 단일 상속만 가능함.
	 *     단, 인터페이스는 다중 상속이 가능함
	 *   - 상속은 extends 라는 키워드를 사용하여 상속을 표현함.
	 *   - 어떤 클래스가 다른 클래스로부터 상속을 받아 만들어지면 새롭게 만들어진 클래스를
	 *    자식 클래스(child or sub)라고 부르며, 멤버 변수와 메서드를 물려준 클래스를 
	 *    부모 클래스(parent or super)라 함.
	 *   - 상속을 하게 되면 부모 클래스의 멤버변수(필드)와 메서드를 자식클래스에 상속하게
	 *    된다. 하지만, 생성자는 상속되지 않는다. 
	 *   - 자바의 모든 클래스는 Object 클래스를 상속받고 있다. Object가 최상위 클래스
	 *   - final 클래스는 상속이 불가능. abstract 클래스는 반드시 상속해서 처리해야 함.
	 *    (abstract 클래스는 추상클래스)  
	 *   
	 *  상속을 잘 사용한 경우와 그렇지 않은 경우
	 *   - 잘못 사용한 경우 : 개별 클래스들이 중복된 속성(멤버변수,필드)/기능(메서드)을 포함하게 된다.
	 *   - 잘 사용한 경우 : 중복된 기능은 빼고, 새로운 클래스를 작성한 후에 구체화
	 *        일반화된 클래스는 부모 클래스로, 구체화된 클래스를 자식클래스로 사용
	 */

	public static void main(String[] args) {
		
		// Person 과 Student, Teacher, Employee의 관계
		// 자식 객체 생성
		...
		// 객체 비교..
		System.out.println("s1 - 이율곡, 15의 해쉬 값 : " + s1.hashCode());
		Student s2 = new Student("이율곡", 10, "program001");
		System.out.println("s2 - 이율곡, 10의 해쉬 값 : " + s2.hashCode());
		Student s3 = new Student("이율곡", 15, "program0001");
		s3.height = 180;
		System.out.println("s3 - 이율곡, 15의 해쉬 값 : " + s3.hashCode());
