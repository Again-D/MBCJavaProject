...
/*
 *  매개변수의 다형성
 *   - promotion은 멤버변수의 값을 대입할 때도 일어나지만, 메서드를 호출할 때 사용하는 매개변수에서도 발생한다.
 *   - 보통 메서드를 호출할 때는 메서드 선언부에 지정한 데이터 타입과 일치하는 매개 값을 전달하여 호출한다.
 *    하지만, 매개변수에 다형성을 적용하면 자식 객체를 전달할 수 있다.
 * 
 * 
 * 
 */
...
public class PolymorphismEx2 {
	...
	public static void main(String[] args) {
		...
		// Instanceof
		//  - 자바 키워드 중 하나로 instanceof 는 객체가 지정한 클래스인지 아닌지 검사할 때 사용
		//	- instanceof는 왼쪽 항의 객체가 오른쪽 항의 클래스인지 검사한다는 의미로 맞으면 True, 틀리면 false
		//	 ex) person instanceof
	  ...
	  public static void printPersonInfo(Person p) {
		  ...
		  /*
		   *  Student 인 경우 배너는 Student Info로, Teacher인 경우 Teacher Info
		   *  Person 인 경우 Person Info로
		   */
