 ...
 * 
 * 인터페이스
 *  - abstract class 와 유사한데 여러개 상속 받을 수 있음. (추상클래스의 단점을 보완)
 *  - 인터페이스에 포함된 메서드들은 모두 abstract method(기본)이다.
...
public interface RemoteControl {
	...
	// 상수 필드(멤버변수) : 인터페이스는 내에 멤필드(멤버변수)설정을 하면 상수 값이 된다.
	// public static final 이 생략됨..
	int MAX_VOLUME = 10;
	int MIN_VOULUME = 0;
	
	// 추상 메서드	-> 구현측에서 반드시 재정의해야 함.
	// 1. 반환타입, 2. 매개변수 타입 과 갯수를 지정, 3. 메소드 이름
	...
	default void setMute(boolean mute) {
		if (mute) {
			...
			setVolume(MIN_VOULUME); // 위에 있는 추상 메서드를 호출...
		}else {
	...
	// 선언시 public 생략했다면, 자동으로 컴파일 과정 중에 public이 붙게됨.
	// 실행부 작성시에 상수 필드(멤버변수)를 제외한 추상 메서드, 디폴트 메서드, private 메서드 등을 호출할 수 없다.(****)
